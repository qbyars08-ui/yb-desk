...
function buildPrompt(ticker, pos, quote) {
  ...
  if (typeof quote?.price === 'number') {
    ...
    context.push('No fresh price available for this ticker. Do NOT state specific current prices, valuation multiples, or revenue figures as fact; describe the setup qualitatively and say plainly what you could not verify.');
  }
...
async function main() {
  ...
  let quote = (prices.quotes ?? {})[ticker];

  // Any-ticker tape: the committed prices.json only covers the book. For
  // everything else, pull a live quote so the brief is anchored to a real
  // price instead of the model's memory. Best effort, never fatal.
  if (typeof quote?.price !== 'number') {
    try {
      const r = await fetch(
        `https://youngbullinvests.com/api/quotes?tickers=${encodeURIComponent(ticker)}`,
        { signal: AbortSignal.timeout(8000) },
      );
      if (r.ok) {
        const data = await r.json();
        const q = data?.quotes?.[ticker];
        if (typeof q?.price === 'number') quote = q;
      }
    } catch { /* offline or API down: brief proceeds without a tape price */ }
  }

  const raw = await callGroq(apiKey, buildPrompt(ticker, pos, quote));
  const cleaned = stripEmDashes(raw).trim();

  // The published brief always carries what was asked, injected
  // deterministically below the title so the model can never drop it.
  // Angle brackets stripped: the question is untrusted visitor input.
  const asked = stripEmDashes(String(process.env.QUESTION || ''))
    .replace(/[<>]/g, '')
    .trim()
    .slice(0, 280);
  const askedBlock = asked ? `**The question on the desk:** ${asked}\n\n` : '';
  const firstBreak = cleaned.indexOf('\n');
  const body =
    askedBlock && firstBreak !== -1 && cleaned.startsWith('#')
      ? `${cleaned.slice(0, firstBreak + 1)}\n${askedBlock}${cleaned.slice(firstBreak + 1)}`
      : `${askedBlock}${cleaned}`;
